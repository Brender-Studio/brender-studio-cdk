...
def calculate_frame_chunk(array_size, job_index, start_frame, end_frame, frame_step):
    ...
    total_frames = end_frame - start_frame + 1  # Ajuste para incluir el frame final
    ...
    end_frame = start_frame + frames_per_chunk + (1 if job_index < extra_frames else 0) - 1  # Ajuste para el frame final
...
def render_animation(start_frame, end_frame, frame_step):
    ...
    for frame in range(int(start_frame), int(end_frame) + 1, int(frame_step)):  # Ajuste para incluir end_frame
